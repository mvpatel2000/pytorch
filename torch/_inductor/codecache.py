        ...
        if key not in cls.cache:
            ...
            with lock:
                ...
                if not os.path.exists(output_so):
                    ...
                    cmd = f"ld -r -b binary -o {consts_o} {consts_path}"
                    run_command_and_check(cmd)
                    ...
                    symbol_list = []
                    symbol_list.append(
                        f"objcopy --redefine-sym _binary_{body}_start=_binary_constants_bin_start {consts_o}"
                    )
                    symbol_list.append(
                        f"objcopy --redefine-sym _binary_{body}_start=_binary_constants_bin_size {consts_o}"
                    )
                    symbol_list.append(
                        f"objcopy --redefine-sym _binary_{body}_end=_binary_constants_bin_end {consts_o}"
                    )
                    log.debug(
                        "aot constant binary redfine symbol: %s", " ".join(symbol_list)
                    )
